...
public class SimHiC_inuse {
	...
	public SimHiC_inuse(String genotype_hdf5, String output_var_file, String output_compounds_file, int num_rounds, 
		...
		this.candidate_regions_locations = SimHiC_inuse.read_candidate_regions_in_compound(input_candidate_genes_file, type);
...
//				System.out.println(c+ " selected_var_indexes_regions2 ["+SNP_index+"] "+selected_var_indexes_region2[SNP_index]);
	...
	public double[][] sim_quantitative_phenotype(String causal_var_file, String simulation_report, String pattern, String sim_pheno){
...
//				    				System.out.println("Round"+pheno_round_index+"\t"+region1_report_string[subj_i][SNP_contribute]);
	...
	public static ArrayList<Integer> balance_case_control_pheno_index(double [][] pheno_matrix,String sim_report) {
	...
	public static void power_caculation(String pfiles_path_file, String pvalue_adjusted_file, String model, String outputfolder, int creteria, String used_var, int w){
		//compound/local/emmax

		try {
			HashMap<String, Double> pvalue_adjusted_hashmap = new HashMap<String, Double>();
			ArrayList<String> pfiles_list = new ArrayList<String>();
			//1st read pfiles into an arraylist
			BufferedReader pfiles_path_fileBufferedReader = new BufferedReader(new FileReader(pfiles_path_file));
			String pfileString = pfiles_path_fileBufferedReader.readLine();
			while(pfileString != null) {
				pfiles_list.add(pfileString);
				pfileString = pfiles_path_fileBufferedReader.readLine();
			}
			int pfiles_len = pfiles_list.size();
			pfiles_path_fileBufferedReader.close();
			//2nd read adjusted pvalues 
			BufferedReader pvalue_adjustedBufferedReader = new BufferedReader(new FileReader(pvalue_adjusted_file));
			String line = pvalue_adjustedBufferedReader.readLine();
			while(line != null) {
				String[]  line_arrStrings = line.split(",");
				pvalue_adjusted_hashmap.put(line_arrStrings[0], Double.parseDouble(line_arrStrings[1]));
				line = pvalue_adjustedBufferedReader.readLine();
			}
			pvalue_adjustedBufferedReader.close();
			
			if (model.equals("compound")) {
				double compound_adjusted_pvalue = pvalue_adjusted_hashmap.get("compound");
				int successful_compounds = 0;
				//read pvalues from each pvalue files
				BufferedReader[] pvalue_files = new BufferedReader[pfiles_len];
				BufferedWriter compound_outputBW = new BufferedWriter(new FileWriter(outputfolder+"cp.power.log"));
				for(int i=0; i<pfiles_len;i++) {
					pvalue_files[i] = new BufferedReader(new FileReader(pfiles_list.get(i)));
					String pvalue_lineString = pvalue_files[i].readLine();
					while (pvalue_lineString != null) {
						if(!pvalue_lineString.startsWith("#")&&!pvalue_lineString.startsWith("compound")) {
							String[] pvalue_lineStrings = pvalue_lineString.split(",");
							double pvalue_of_this_compound = Double.parseDouble(pvalue_lineStrings[1]);
							if(Double.compare(pvalue_of_this_compound, compound_adjusted_pvalue)<=0) {
								successful_compounds+=1;
								compound_outputBW.write(i+"th file's"+"\tsuccessful #"+successful_compounds+"\t"+pvalue_lineStrings[0]+"\n");
							}
						}
						pvalue_lineString = pvalue_files[i].readLine();
					}
					pvalue_files[i].close();
				}
				
				double power = Double.valueOf(successful_compounds)/ pfiles_len;
				compound_outputBW.write("The power of "+pfiles_len+" pvalue files for model "+model+" is: "+power);
				compound_outputBW.close();
			}else if (model.equals("local")) {
				double adjusted_pvalue = pvalue_adjusted_hashmap.get("local");
				local_power(pfiles_list, adjusted_pvalue, model, outputfolder, creteria,used_var,w);
			}else if (model.equals("emmax")){
				double adjusted_pvalue = pvalue_adjusted_hashmap.get("emmax");
				emmax_power(pfiles_list, adjusted_pvalue, model, outputfolder, creteria, used_var);
			}else if(model.equals("skat")) {
//				skat_power();
				double adjusted_pvalue = pvalue_adjusted_hashmap.get("skat");
				local_power(pfiles_list, adjusted_pvalue, model, outputfolder, creteria,used_var,w);
			}
		}catch(Exception e) {
			e.printStackTrace();
		...
		if (args.length == 0) {
			...
			if(function.equals("simulation")) {
				if(args.length<number_of_parameter*2+1) {
					...
					SimHiC_inuse sim_hic = new SimHiC_inuse(genotype_hdf5, output_var_file, output_compounds_file, num_rounds, min_maf, max_maf, num_causal_compounds, 
				...
				if(args.length<num_parameter*2+1) {
					System.out.print("Usage:\n"
							+ "\t<-p_path_p\ta file contains all pvalues of a certain model>\n"
							+ "\t<-padj\ta file contains adjusted pvalues for models>\n"
							+ "\t<-model\tmodel(compound/local/emmax/skat)>\n"
							+ "\t<-used_var\t a file path contain all variants\n"
							+ "\t<-of\t the output folder contains power files for models>\n"
							+ "\t<-cre creteria for local and emmax power evaluation, 1 for one region success, 2 for two regions success>\n"
							+ "\t<-w window length for local and skat, 0 for compound and Emmax");
				}else {
					String pfiles_path_file=null;
				    String pvalue_adjusted_file=null;
					String model=null;
					...
					for(int k=1; k<args.length;k++) {
						if(args[k].startsWith("-")) {
							if(args[k].startsWith("-p_path_p")) pfiles_path_file=args[k+1];
							else if(args[k].startsWith("-padj")) pvalue_adjusted_file=args[k+1];
							else if(args[k].startsWith("-model")) model=args[k+1];
							else if(args[k].startsWith("-of")) outputfolder=args[k+1];
							else if(args[k].startsWith("-cre")) creteria=Integer.parseInt(args[k+1]);
							else if(args[k].startsWith("-used_var")) used_var=args[k+1];
							else if(args[k].startsWith("-w")) w=Integer.parseInt(args[k+1]);
						}
					}
					SimHiC_inuse.power_caculation(pfiles_path_file, pvalue_adjusted_file,model,outputfolder,creteria,used_var,w);
				}
			}else {
